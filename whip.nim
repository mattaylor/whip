...
type Handler* = proc(r: Wreq): Future[void] {.inline, closure, gcsafe.}
...
proc send*(my: Wreq, body: string, head = TEXT_TYPE): Future[void] {.inline,
    gcsafe.} = my.req.send(Http200, body, head)

proc send*[T](my: Wreq, body: T, head = TEXT_TYPE): Future[void] {.inline,
    gcsafe.} = my.req.send(Http200, $body, head)

proc send*[T](my: Wreq, body: Future[T], head = TEXT_TYPE) {.async, inline,
    gcsafe.} = asyncCheck my.send(await(body), head)

proc html*(my: Wreq, body: string): Future[void] {.inline,
    gcsafe.} = my.req.send(Http200, body, HTML_TYPE)

proc html*(my: Wreq, body: Future[string]) {.async, inline,
    gcsafe.} = my.req.send(Http200, await(body), HTML_TYPE)

proc json*(my: Wreq, body: JsonNode): Future[void] {.inline,
    gcsafe.} = my.req.send(Http200, $body, JSON_TYPE)

proc json*(my: Wreq, body: string): Future[void] {.inline,
    gcsafe.} = my.req.send(Http200, body, JSON_TYPE)

proc json*(my: Wreq, body: Future[string]) {.async, inline,
    gcsafe.} = my.req.send(Http200, await(body), JSON_TYPE)

proc json*[T](my: Wreq, body: T): Future[void] {.inline, gcsafe.} = my.req.send(
    Http200, $(%body), JSON_TYPE)
...
func initWhip*(): Whip {.inline.} =
  let w = Whip(router: newRouter[Handler](), simple: newTable[HttpMethod,
      CritBitTree[Handler]]())
...
proc start*(my: Whip, port: int = 8080) =
  ...
  run(proc (beast: Request): Future[void] =
    ...
    sim[req.uri.path] = proc(r: Wreq): Future[void] {.inline, closure.} =
